...
internal class StateEngine(engine: CombatEngine) {
    private val stateMap: Map<EngineState, State> = mapOf(
        EngineState.PlayerControl to
                PlayerControlState(
                    engine.hand, engine.discarder, engine.energy,
                    engine.playerActor.character, engine.enemyActor.character
                ),
        EngineState.Discarding to
                DiscardingState(engine.hand, engine.discarder),
        EngineState.Drawing to
                DrawingState(engine.hand, engine.drawPile, engine.discardPile, engine.discarder),
        EngineState.EnemyTurn to
                EnemyTurnState(engine.playerActor, engine.enemyActor),
        EngineState.Done to
                DoneState(engine.discarder)
    )
